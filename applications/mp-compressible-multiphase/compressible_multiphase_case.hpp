...
namespace MeltPoolDG::Multiphase
{
  ...
  protected:
                        ...
                        const std::string         &norm_name = "Norm") const
    {
      ...
      const unsigned int n_dofs_per_element_per_phase = 0.5 * dof_handler.get_fe_collection().max_dofs_per_cell();
      ...
      auto compute_error_norm = [&](auto &fe_point_eval, auto &physical_domain, const bool is_gas_phase) {
        ...
        for (const auto &cell : dof_handler.active_cell_iterators())
          {
            ...
            if (n_dofs_per_cell != n_dofs_per_element_per_phase)
              {
                // For an intersected element, we have to extract the Dofs, which are relevant for the current phase
                std::vector<number> solution_values_in_tmp(n_dofs_per_cell);
                cell->get_dof_values(dof_vector, solution_values_in_tmp.begin(), solution_values_in_tmp.end());

                if (is_gas_phase)
                  solution_values_in.assign(solution_values_in_tmp.begin() + n_dofs_per_element_per_phase, solution_values_in_tmp.end());
                else
                  solution_values_in.assign(solution_values_in_tmp.begin(), solution_values_in_tmp.begin() + n_dofs_per_element_per_phase);
              }
            else
            ...
            for (const unsigned int q : fe_point_eval.quadrature_point_indices())
              {
                ...
                for (unsigned int i = 0; i < dim+2; ++i)
                  {
                    error[i] = VectorTools::evaluate_function_at_vectorized_points<dim, number>(
