...
"""
 Usage: python3 compare_figure_4.py
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot the comparison figure for fig. 4 in Zahedi (2009).")
    ...
    parser.add_argument("-fig", "--figure_name", type=str, default="zahedi_comparison_figure4.png",
    ...
    zahedi_results_folder = os.path.join(os.getcwd(), f"{args.zahedi_results_folder_path}/fig4/")
    ...
    label_list = [r"$\epsilon_\text{t} = \ \ \ \ 3\epsilon_\text{n}$ ", r"$\epsilon_\text{t} = \ \ 12\epsilon_\text{n}$ ",
                  r"$\epsilon_\text{t} = \ \ 48\epsilon_\text{n}$ ", r"$\epsilon_\text{t} = 192\epsilon_\text{n}$ "]
    ...
    for i in range(1, 5):
        ...
        sim_time = [t for t in sim_time if (t > 1e-6)]
        n_removed_data_points = len(sim_contact_angle) - len(sim_time)
        sim_contact_angle = sim_contact_angle[n_removed_data_points:]
        ...
        """
         Plot data in figure
        """
        ax1.plot(sim_time, sim_contact_angle, lw=linewidth_list[0], color=colors_list[2 * j],
                 ls=linestyle_list[0], label=label_list[j], marker=marker_list[0], ms=6, markerfacecolor='none')
        ax1.plot(zahedi_df['time'], zahedi_df['contact_angle'], lw=linewidth_list[1],
                 color=colors_list[2 * j + 1], ls=linestyle_list[1])

    x_min = zahedi_df['time'].iloc[0]
    x_min = min(np.min(sim_time), x_min)
