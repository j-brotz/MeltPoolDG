...
namespace
{
  ...
  {
    using ObstacleType = SphericalParticle<dim, double>;
  ...
  public:
    ...
    {
      /**
       * Constructor that sets up all necessary components required for finally setting up
      ...
      std::unique_ptr<CompressibleFlow::DGOperatorExplicit<dim, number, 1>> flow_operator;
    ...
    private:
      ...
      setup_scratch_data()
      {
        ...
        flow_operator = std::make_unique<CompressibleFlow::DGOperatorExplicit<dim, number, 1>>(
          *flow_scratch_data);
      }
  ...
  /**
   * @brief Benchmark for the apply_operator() method of the compressible flow explicit operator with
  ...
  (benchmark::State &state)
  {
    ...
    ObstacleField<dim, double, SphericalParticle<dim, double>> obstacle_field(
      obstacle_data,
    ...
    BrinkmanPenalizationData<double> brinkman_data;
    auto                             matrix_free_cell_batch_particle_cache =
      std::make_shared<Particles::MatrixFreeCellBatchParticleCache<dim, double>>(
        MatrixFreeContext<dim, double>(this->data->scratch_data.get_matrix_free(),
    ...
    MeltPoolDG::BrinkmanObstacleForce<dim, double, SphericalParticle<dim, double>>
    ...
    auto fsi_fluid_force_residual = std::make_shared<
      BrinkmanPenalizationResidualContribution<dim, double, SphericalParticle<dim, double>>>(
