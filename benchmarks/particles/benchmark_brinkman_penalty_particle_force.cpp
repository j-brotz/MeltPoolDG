 ...
 * particles for a given flow field solution. The benchmark sets up a simple rectangular domain with
 * a specified number of particles, and then computes the forces acting on the particles using the
 * Brinkman penalization method. The number of particles is defined by the parameter @p state.range
 * at index 0 and can be varied for different benchmark runs.
 */
void
BM_BrinkmanPenaltyParticleForce(benchmark::State &state)
{
  ...
  auto matrix_free_cell_batch_particle_cache =
    std::make_shared<Particles::MatrixFreeCellBatchParticleCache<dim, number>>(
      MatrixFreeContext<dim, number>(scratch_data.get_matrix_free(), 0, 0));
