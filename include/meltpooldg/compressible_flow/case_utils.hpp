...
namespace MeltPoolDG::CompressibleFlow
{
  ...
  struct InputDefinedFreeJetInflow
  {
    ...
    /// Boolean flag to determine whether the free jet inflow boundary condition should be applied
    /// at the specified boundary.
    bool set_free_jet_inflow{false};

    /// Jet-hole diameter
    number jet_hole_diameter;

    /// Jet-hole density at the inflow boundary
    number jet_hole_density;

    /// Jet-hole specific inner energy at the inflow boundary
    number jet_hole_inner_energy;
