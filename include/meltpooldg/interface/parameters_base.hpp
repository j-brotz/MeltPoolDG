...
namespace MeltPoolDG
{
  ...
  /**
   * @brief Utility function to print parameters from a ParameterHandler object.
   *
   * This function formats and prints the parameters managed by a `ParameterHandler`
   * to a given output stream in either JSON or ShortJSON style, based on the `print_details` flag.
   *
   ...
   * @param print_details If `true`, prints parameters in detailed JSON format.
   *                      Otherwise, prints a shorter JSON format.
   */
  void
  ...
  public:
    ...
    /**
     * @brief Determine if parameters should be printed.
     *
     * Derived classes must implement this to specify whether parameter printing is enabled.
     *
     * @return `true` if parameter printing is enabled, `false` otherwise.
     */
    virtual bool
    enable_print_parameters() const = 0;
