...
namespace MeltPoolDG
{
  ...
  template <int dim, typename number, typename ObstacleType, typename ContainerType>
    requires std::ranges::range<ContainerType> and
             std::same_as<std::ranges::range_value_t<ContainerType>,
                          DEMParticleAccessor<dim, number>>
  void
  symplectic_euler_advance_time_step(const number time_step, ContainerType particle_range)
  {
