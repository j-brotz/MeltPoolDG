...
namespace MeltPoolDG
{
  ...
  void
  AMR::BinaryOpIndicatorComposite<dim, number, BinaryOp>::add_indicator(
  ...
  template <int dim, typename number, template <typename> class BinaryOp>
  auto
  AMR::BinaryOpIndicatorComposite<dim, number, BinaryOp>::compute_indicator(
  ...
  template <int dim, typename number, template <typename> class BinaryOp>
  bool
  AMR::BinaryOpIndicatorComposite<dim, number, BinaryOp>::empty() const
  {
    return indicators_and_weights.empty();
  }

  template <int dim, typename number, int n_dof_components, int n_indicator_components>
  AMR::SSEDIndicator<dim, number, n_dof_components, n_indicator_components>::SSEDIndicator(
  ...
  AMR::SSEDIndicator<dim, number, n_dof_components, n_indicator_components>::compute_indicator(
  ...
  template <int dim, typename number, int n_dof_components, int n_indicator_components>
  void
  AMR::SSEDIndicator<dim, number, n_dof_components, n_indicator_components>::local_apply_cell(
  ...
  template <int dim, typename number, int n_dof_components, int n_indicator_components>
  AMR::JumpIndicator<dim, number, n_dof_components, n_indicator_components>::JumpIndicator(
    const MatrixFreeContext<dim, number>    matrix_free_context,
    const VectorType                       &solution,
    const ExtractIndicatorDofValuesFunction extract_indicator_dof_values,
    const unsigned                          fe_index)
  ...
  template <int dim, typename number, int n_dof_components, int n_indicator_components>
  dealii::Vector<number>
  AMR::JumpIndicator<dim, number, n_dof_components, n_indicator_components>::compute_indicator(
    const dealii::Triangulation<dim> &tria)
  {
    ...
    LocalOpType face_op = std::bind_front(
      &JumpIndicator<dim, number, n_dof_components, n_indicator_components>::local_apply_face,
      this);
  ...
  template <int dim, typename number, int n_dof_components, int n_indicator_components>
  void
  AMR::JumpIndicator<dim, number, n_dof_components, n_indicator_components>::local_apply_face(
    const dealii::MatrixFree<dim, number> &,
    dealii::AlignedVector<VectorizedArrayType> &dst,
    const VectorType                           &src,
    ...
    for (unsigned face = face_range.first; face < face_range.second; ++face)
      {
        ...
        for (const unsigned q : phi_m.quadrature_point_indices())
          {
            const auto w_p =
              extract_indicator_dof_values(fe_evaluation_tensor_value_at_q(phi_p, q));
            const auto w_m =
              extract_indicator_dof_values(fe_evaluation_tensor_value_at_q(phi_m, q));
