...
# collect json files
json_files = [os.path.join(dp, f) for dp, dn, filenames in os.walk(os.path.join(root_dir,"tests")) for f in filenames if os.path.splitext(f)[1] == '.json']
json_files += [os.path.join(dp, f) for dp, dn, filenames in os.walk(os.path.join(root_dir,"simulations")) for f in filenames if os.path.splitext(f)[1] == '.json']
  ...
  for a, b in d.items():
    if b:
      if isinstance(b, dict):
        ...
        final_dict[a] = list(filter(None, [remove_empty_items(i) for i in b]))
      else:
        final_dict[a] = b
  return final_dict
    ...
    for j in json_f:
        with open(j, 'r') as f:
            ...
            for i, o in enumerate(old_parameter_names):
                ...
                print("    Rename OLD {:} to NEW {:}".format(o, new_parameter_names[i]))
