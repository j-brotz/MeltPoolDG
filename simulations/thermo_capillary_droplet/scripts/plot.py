...
if __name__ == "__main__":
    ...
    ax.scatter(analytical_t,analytical_vel, color='k', label='analytical solution')
    ax.legend()
    ax.grid()
    ax.set_ylabel("u/ur")
    ax.set_xlabel("t/tr")
    ...
    ax.set_xlabel("t/tr")
    ax.set_xlim([0,2])
    ax.scatter([time_normalized[-100], time_normalized[-1]], [center_position[-100], center_position[-1]])
    ax.plot([time_normalized[-100], time_normalized[-1], time_normalized[-1]], [center_position[-100], center_position[-100], center_position[-1]], "k")
    vel = (center_position[-1]- center_position[-500])/(time[-1] -time[-500])/0.024
    ax.annotate("u/ur={:}".format(float(vel)), (time_normalized[-500], center_position[-500]))
    
    plt.tight_layout()
    plt.show()
    plt.savefig(data_file[:-4]+".png")
