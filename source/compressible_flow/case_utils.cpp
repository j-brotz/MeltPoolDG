  ...
  InputDefinedFreeJetInflow<dim, number>::add_parameters(dealii::ParameterHandler &prm)
  {
    prm.enter_subsection("free jet");
    {
      prm.add_parameter(
        "set free jet inflow",
        set_free_jet_inflow,
        "Boolean flag to determine whether the free jet inflow boundary condition should be applied at the specified boundary. Note that this is only relevant for inflow boundaries, and that the free jet inflow is applied at the boundary if this flag is set to true and if the boundary type is set to 'inflow'.");
