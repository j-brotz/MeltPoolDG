...
namespace MeltPoolDG::Multiphase
{
                         ...
                         const std::pair<unsigned, unsigned> &cell_range) const
  {
    ...
    auto phi_liquid             = create_cell_integrator(CutUtil::CellCategory::liquid, 0);
    auto phi_gas                = create_cell_integrator(CutUtil::CellCategory::gas, dim + 2);
    auto phi_liquid_intersected = create_cell_integrator(CutUtil::CellCategory::intersected, 0);
    auto phi_gas_intersected = create_cell_integrator(CutUtil::CellCategory::intersected, dim + 2);
    ...
    auto process_cell = [&]<bool is_gas_phase, bool is_viscous, typename T0>(T0 &phi) {
      for (const unsigned int q : phi.quadrature_point_indices())
        {
          auto [flux, grad_flux] =
            MeltPoolDG::Flow::rhs_cell_integral_kernel<dim, number, T0, is_viscous, is_gas_phase>(
              phi,
              q,
    ...
    switch (active_fe_index)
      {
            ...
            for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
              {
                ...
                phi_liquid.integrate_scatter(EvaluationFlags::values | EvaluationFlags::gradients,
                                             dst);
              }
            break;
          }
            ...
            for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
              {
                ...
                phi_gas.integrate_scatter(EvaluationFlags::values | EvaluationFlags::gradients,
                                          dst);
              }
            break;
          }
          case CutUtil::CellCategory::intersected: {
            ...
            dealii::FEPointEvaluation<dim + 2, dim, dim, dealii::VectorizedArray<number>>
              phi_point_liquid(*mapping_info_cells[0], fe_point_temp);
            dealii::FEPointEvaluation<dim + 2, dim, dim, dealii::VectorizedArray<number>>
              phi_point_surface_liquid(mapping_info_surface, fe_point_temp);
            dealii::FEPointEvaluation<dim + 2, dim, dim, dealii::VectorizedArray<number>>
              phi_point_gas(*mapping_info_cells[1], fe_point_temp);
            dealii::FEPointEvaluation<dim + 2, dim, dim, dealii::VectorizedArray<number>>
                 ...
                 ++cell_batch)
              {
                ...
                for (unsigned int lane = 0; lane < flow_scratch_data.scratch_data.get_matrix_free()
                     ...
                     ++lane)
                  {
                    ...
                    phi_point_liquid.evaluate(StridedArrayView<const number, n_lanes>(
                    ...
                    phi_point_surface_liquid.evaluate(
                    ...
                    phi_point_surface_gas.evaluate(
                      StridedArrayView<const number, n_lanes>(
                        &phi_gas_intersected.begin_dof_values()[0][lane], n_dofs_per_cell),
                      EvaluationFlags::values | EvaluationFlags::gradients);
                    ...
                    // do surface integral
                    switch (flow_scratch_data.flow_data.multiphase.interface_numerical_method)
                      {
                          case InterfaceNumericalMethod::penalty: {
                            for (const unsigned int q :
                                 phi_point_surface_liquid.quadrature_point_indices())
                              {
                                ...
                                auto flux_data =
                                  calculate_interface_flux_penalty<dim,
                                                                   number,
                          ...
                          case InterfaceNumericalMethod::HLLP0_and_Nitsche: {
                            for (const unsigned int q :
                                 phi_point_surface_gas.quadrature_point_indices())
                              {
                                auto w_liquid      = phi_point_surface_liquid.get_value(q);
                                auto w_gas         = phi_point_surface_gas.get_value(q);
                                auto grad_w_liquid = phi_point_surface_liquid.get_gradient(q);
                                ...
                                auto riemann_flux_data = calculate_convective_interface_flux_HLLP0<
                                  dim,
                                  number,
                                  ConservedVariablesType,
                                  ConservedVariablesGradType>(
                                  ...
                                  w_gas,
                                  normal,
                                  flow_scratch_data.flow_data.multiphase.m_dot_evap,
                                  convective_terms,
                                  flow_scratch_data.flow_data);

                                auto flux_liquid = std::get<0>(riemann_flux_data);
                                auto flux_gas    = -std::get<1>(
                                  riemann_flux_data); // opposite normal direction for phase 2
                                ...
                                if (is_viscous_liquid or is_viscous_gas)
                                  {
                                    auto viscous_interface_flux_data =
                                      calculate_viscous_interface_flux<dim,
                                        ...
                                        alpha_2,
                                        flow_scratch_data.flow_data.multiphase.m_dot_evap,
                                        flow_scratch_data.flow_data.multiphase
                                          .symm_int_penalty_parameter_interface,
                                        viscous_terms,
                                        flow_scratch_data.flow_data);

                                    flux_liquid -= viscous_interface_flux_data.first;
                                    flux_gas += viscous_interface_flux_data
                                                  .second; // opposite normal direction for phase 2
                                  }
                                ...
                                if (is_viscous_liquid or is_viscous_gas)
                                  {
                                    ...
                                    phi_point_surface_liquid.submit_gradient(
                                      -numerical_flux_gradient.first, q);
                                    phi_point_surface_gas.submit_gradient(
                                      -numerical_flux_gradient.second, q);
                                  }
                              }
                            break;
                          }
                    ...
                    phi_point_surface_liquid.integrate(StridedArrayView<number, n_lanes>(
                                  &phi_liquid_intersected.begin_dof_values()[0][lane],
                                  n_dofs_per_cell),
                                  EvaluationFlags::values | (flow_scratch_data.flow_data.multiphase.interface_numerical_method == InterfaceNumericalMethod::HLLP0_and_Nitsche ? EvaluationFlags::gradients : EvaluationFlags::nothing) , true
                         ...
                         const std::pair<unsigned int, unsigned int> &face_range) const
  {
    ...
    auto phi_gas_p    = create_face_integrator(false, CutUtil::CellCategory::gas, dim + 2);
    auto phi_liquid_m_intersected =
    ...
    auto process_intersected_face =
      [&]<bool is_gas_phase, bool is_viscous>(auto              &phi_m_int,
                                              auto              &phi_p_int,
        ...
        for (unsigned int face = face_range.first; face < face_range.second; ++face)
          {
            ...
            phi_m_int.project_to_face(eval_flags);
            phi_p_int.project_to_face(eval_flags);
            ...
            for (unsigned int lane = 0;
                 ...
                 ++lane)
              {
                ...
                phi_point_m.evaluate_in_face(&phi_m_int.get_scratch_data().begin()[0][lane],
                                             eval_flags);

                phi_point_p.evaluate_in_face(&phi_p_int.get_scratch_data().begin()[0][lane],
                                             eval_flags);

                // factor 0.5 for interior face
                const dealii::VectorizedArray<number> penalty_parameter =
                  is_viscous ? 0.5 * std::max(phi_m_int.read_cell_data(
                                                flow_scratch_data.interior_penalty_parameter),
                                              phi_p_int.read_cell_data(
                                                flow_scratch_data.interior_penalty_parameter)) :
                               0.;
                ...
                phi_point_m.integrate_in_face(&phi_m_int.get_scratch_data().begin()[0][lane],
                                              eval_flags);

                phi_point_p.integrate_in_face(&phi_p_int.get_scratch_data().begin()[0][lane],
                                              eval_flags);
              }

            phi_m_int.collect_from_face(eval_flags, phi_m_int.begin_dof_values());
            phi_p_int.collect_from_face(eval_flags, phi_p_int.begin_dof_values());
                                  ...
                                  const std::pair<unsigned, unsigned> &face_range) const
  {
      ...
      for (unsigned int face = face_range.first; face < face_range.second; ++face)
        {
          ...
          const dealii::VectorizedArray<number> penalty_parameter =
            is_viscous ? phi_m.read_cell_data(flow_scratch_data.interior_penalty_parameter) : 0.;
    ...
    auto process_intersected_face = [&]<bool is_gas_phase, bool is_viscous>(
                                      auto &phi_m_int, const unsigned int mapping_idx) {
    ...
    switch (face_category.first)
      {
          ...
          case CutUtil::CellCategory::intersected: {
            process_intersected_face.template operator()<false, is_viscous_liquid>(
              phi_liquid_m_intersected, 0);
            process_intersected_face.template operator()<true, is_viscous_gas>(
              phi_gas_m_intersected, 1);
            break;
          }
        default:
          break;
