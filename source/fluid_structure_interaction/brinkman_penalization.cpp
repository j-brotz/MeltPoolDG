  ...
  ObstacleField<dim, number, ObstacleType> &obstacle_field) const
{
  ...
  for (auto &locally_owned_particle : obstacle_field.locally_owned_particle_range())
    {
      for (unsigned int src_handle = 0;
           src_handle < global_particle_properties.n_registered_slots();
           ++src_handle)
        {
          if (locally_owned_particle.id() == global_particle_properties.get_properties(
                                               src_handle)[ObstacleType::Properties::particle_id])
            {
              locally_owned_particle.add_force(
                ObstacleType::get_force(global_particle_properties, src_handle));
              locally_owned_particle.add_torque(ObstacleType::get_torque(global_particle_properties,
                                                                         src_handle));
            }
        }
    }
