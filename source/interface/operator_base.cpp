...
{
  template <typename number, typename DoFVectorType, typename SrcRhsVectorType>
  void
  OperatorBase<number, DoFVectorType, SrcRhsVectorType>::set_time_increment(const double dt)
  {
    d_tau     = dt;
    d_tau_inv = 1. / d_tau;
  }
  ...
  template <typename number, typename DoFVectorType, typename SrcRhsVectorType>
  void
  OperatorBase<number, DoFVectorType, SrcRhsVectorType>::reset_indices(
  ...
  template <int dim>
  void
  OperatorBase<number, DoFVectorType, SrcRhsVectorType>::initialize_matrix_based(
  ...
  template <int dim>
  void
  OperatorBase<number, DoFVectorType, SrcRhsVectorType>::create_rhs_and_apply_dirichlet_mf(
    DoFVectorType &         rhs,
  ...
  template <typename number, typename DoFVectorType, typename SrcRhsVectorType>
  const OperatorBase<number, DoFVectorType, SrcRhsVectorType>::SparseMatrixType &
  OperatorBase<number, DoFVectorType, SrcRhsVectorType>::get_system_matrix() const
  {
