...
#include <meltpooldg/particles/obstacle_data_structure_arborx.hpp>
...
namespace
{
  ...
  protected:
      ...
      , mapping(1)
    {
      ...
      obstacle_data_structure =
        std::make_unique<MeltPoolDG::ObstacleDataStructureArborX<dim, double, ObstacleType>>(
    ...
    insert_particle(const ParticleSpecification &spec)
    {
      ...
      obstacle_data_structure->insert_obstacles(
    ...
    std::unique_ptr<MeltPoolDG::ObstacleDataStructureArborX<dim, double, ObstacleType>>
      obstacle_data_structure;
  ...
  TEST_F(ObstacleDataStructureFixture, GetParticlesInCellSingleDistributedParticle)
  {
    ...
    std::cout << "MPI Rank: "
              << dealii::Utilities::MPI::this_mpi_process(triangulation.get_mpi_communicator())
              << ", n local particles: "
