...
namespace
{
  ...
  protected:
    ...
    ContactForceFixture()
      : mapping(1)
      , contact_data({.restitution_coefficient      = 1.0,
    ...
    insert_particle(const ParticleTestData &particle_data)
    {
      ...
      properties[ObstacleType::Properties::moment_of_inertia] =
        dim == 3 ? (2.0 / 5.0) * properties[ObstacleType::Properties::mass] * particle_data.radius *
                     particle_data.radius :
                   (1.0 / 2.0) * properties[ObstacleType::Properties::mass] * particle_data.radius *
                     particle_data.radius;
  ...
  TEST_F(ContactForceFixture, ParticleParticleElasticNormalImpact)
  {
    ...
    auto   post_processor                  = [&]() {
      max_normal_contact_force =
        std::max(max_normal_contact_force, std::abs(obstacle_field->begin()->get_force().norm()));

      auto particle_iterator = obstacle_field->begin();
      ...
      if (2 * particle_radius - std::abs(loc_particle2[0] - loc_particle1[0]) > 0)
        contact_duration += time_iterator.get_current_time_increment();
    };

    time_loop(time_iterator, *obstacle_field, post_processor);
    ...
    double helper_factor =
      std::pow(5 * std::numbers::sqrt2_v<double> * particle_density * std::numbers::pi_v<double> *
                 ...
                 (4. * contact_data.particle.youngs_modulus),
               0.4);
    ...
    double expected_max_normal_contact_force =
      std::sqrt((2. * particle_radius * contact_data.particle.youngs_modulus *
                 contact_data.particle.youngs_modulus) /
                (9. * std::pow(1 - contact_data.particle.poisson_ratio *
